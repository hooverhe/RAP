...
(function() {
    ...
    */
    ...
    ws.showMockData = function(actionId,mockType) {
        var action = p.getAction(actionId);
        action.mockType=mockType;
        ...
        function getAInfoHtml(a) {
            var head = "<h2 style='margin-top:20px;'>接口详情 <span style='font-size: 14px; color: #999;'>(id: " + a.id 
                + ") &nbsp;&nbsp;<button class=\"btn btn-danger btn-xs\" onclick=\"ws.showMockData("
                    + a.id + ",1);\">MockRequest数据</button>&nbsp;&nbsp;<button class=\"btn btn-danger btn-xs\" onclick=\"ws.showMockData("
                + a.id + ",2);\">MockResponse数据</button></span> </h2><div class='action-info' href='#' onclick='ws.editA("
                + a.id + "); return false;'>",
                body = "",
                foot = "</div>";
